if (!window.hasRunOpenDyslexicScript) {
    ...
    function applyFont(fontFamily, fontSize) {
        const css = `* { font-family: '${fontFamily}' !important; font-size: ${fontSize}px !important; line-height: 1.5 !important; }
    ...
    function initializeFontSettings() {
        chrome.storage.local.get(['fontEnabled', 'selectedFont', 'fontSize'], (data) => {
            if (data.fontEnabled) applyFont(data.selectedFont || 'OpenDyslexic-Regular', data.fontSize || '16');
        });
    }
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        try {
            if (message.fontEnabled !== undefined) {
                message.fontEnabled ? applyFont(message.selectedFont || 'OpenDyslexic-Regular', message.fontSize || '16') : removeFont();
                chrome.storage.local.set({ fontEnabled: message.fontEnabled, selectedFont: message.selectedFont, fontSize: message.fontSize });
            } else if (message.selectedFont || message.fontSize) {
                applyFont(message.selectedFont, message.fontSize);
                chrome.storage.local.set({ selectedFont: message.selectedFont, fontSize: message.fontSize });
            }
